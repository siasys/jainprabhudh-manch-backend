...
// ---------------------------------------------------------------------------
// MEMBER ADDRESS SUB-SCHEMA
// strict:false -> country ke apne field names bhi save honge, bilkul waise
// jaise Jain Shravak ki location me:
//   Canada -> province, region, postal_code
//   USA    -> state, county, zip_code
//   Japan  -> prefecture, postal_code
// street/city/district/state/pincode declared rehte hain kyunki member card,
// letterhead aur edit form sab unhi ko padhte hain.
// SIRF members aur honoraryMembers par lagta hai -- officeBearers/panches/
// sanghTeams ka address purane data me kabhi-kabhi plain STRING hota hai
// (addOfficeBearer form se seedha string bhejta hai), aur sub-schema us
// string par "Tried to set nested object field" error deta hai.
// India ka behaviour bilkul pehle jaisa hai.
// ---------------------------------------------------------------------------
...
const memberSchema = new mongoose.Schema({
  ...
  // ===== FY renewal (additive) =====
  lastPaidFY: { type: String, default: null },
  paymentHistory: [
    {
...
const hierarchicalSanghSchema = new mongoose.Schema(
  {
    ...
    location: {
      ...
      state: {
        ...
        required: function () {
          return (
            (this.location?.country || "India") === "India" &&
            ["state", "district", "city", "area"].includes(this.level)
          );
        },
      },
      district: {
        ...
        required: function () {
          return (
            (this.location?.country || "India") === "India" &&
            ["district", "city", "area"].includes(this.level)
          );
        },
      },
      city: {
...
hierarchicalSanghSchema.methods.validateHierarchy = async function () {
  ...
  // International: country ke upar ka level. Parent foundation hota hai,
  // ya isi level ka main Sangh (women/youth/veerSena ke liye).
  if (this.level === "international") {
    ...
    const isSameLevelSpecialized =
      parentSangh.level === "international" &&
      parentSangh.sanghType === "main" &&
      ["women", "youth", "veerSena"].includes(this.sanghType);

    if (!isSameLevelSpecialized) {
      throw new Error(
        "International level Sangh must have a foundation as parent or be a specialized Sangh under international main Sangh",
      );
    }
  ...
  if (this.level === "country") {
    ...
    // Allow foundation ya international as parent
    if (
      parentSangh.level === "foundation" ||
      parentSangh.level === "international"
    ) {
      return;
    }
    ...
    if (!isSameLevelSpecialized) {
      throw new Error(
        "Country level Sangh must have a foundation/international parent or be a specialized Sangh under country main Sangh",
      );
    }
  ...
  // FIX: isSameLevelSpecialized upar sirf country-block ke andar defined tha.
  //      Non-country levels (state/district/city) par ye undefined ho jaata tha
  //      aur "isSameLevelSpecialized is not defined" crash aata tha.
  //      Yahan is scope me define karo — parent main Sangh ke SAME level par
  //      specialized (women/youth/veerSena) sangh allowed hai.
  const isSameLevelSpecialized =
  ...
  if (currentIndex <= parentIndex && !isSameLevelSpecialized) {
    throw new Error(
      `Invalid hierarchy: ${this.level} level cannot be directly under ${parentSangh.level} level`,
    );
  }
};
