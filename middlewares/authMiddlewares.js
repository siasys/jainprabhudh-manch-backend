...
const authenticate = async (req, res, next) => {
  try {
    ...
    try {
      ...
      // Suspended accounts cannot use the app at all
...
const canReviewJainAadhar = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    (user.role === "admin" &&
      user.adminPermissions.includes("verify_jain_aadhar"))
  ) {
  ...
  const presidentRole = user.sanghRoles?.find(
    (role) =>
      role.role === "president" &&
...
const checkAccess = asyncHandler(async (req, res, next) => {
  ...
  if (user.jainAadharStatus === "pending") {
    return res.status(403).json({
      success: false,
      message:
        "Your Jain Aadhar verification is pending. Some features may be limited.",
      status: "pending",
    });
  }
  ...
  return res.status(403).json({
    success: false,
    message:
      "Trial period expired. Please verify your Jain Aadhar to continue using all features.",
    status: "expired",
  });
});
...
const verifySanghRole = asyncHandler(async (req, res, next) => {
  ...
  const hasSanghRole =
    user.sanghRoles &&
    user.sanghRoles.some((role) => role.sanghId.toString() === sanghId);
...
const verifyPanchRole = asyncHandler(async (req, res, next) => {
  ...
  const hasPanchRole =
    user.panchRoles &&
    user.panchRoles.some((role) => role.panchId.toString() === panchId);
...
const verifyTirthRole = asyncHandler(async (req, res, next) => {
  ...
  const hasTirthRole =
    user.tirthRoles &&
    user.tirthRoles.some((role) => role.tirthId.toString() === tirthId);
